...
초보자 기준으로 정리해 보았습니다. 우린 병아리였던 적이 있었조!
...
**파일 유형별 폴더**로 나누는 패턴을 소개합니다. 

Chirpy 커뮤니티에서도 정적 이미지를 `_posts`가 아니라  
`assets/img` 아래에 두는 방식을 많이 사용합니다. 
...
Jekyll 사용자들 사이에서도  
포스트마다 해당 자산을 모아 두는 구조가 오래전부터 추천 패턴으로 쓰이고 있습니다. 
...
Chirpy 문서는 `image`를 문자열 또는 `image.path` + `alt` 형식으로 사용하는 방식을 안내합니다. 
...
본문 이미지는 Markdown 문법으로 글 안에 직접 넣습니다.
...
포스트별 이미지 경로를 단순하게 관리하는 방법을 소개합니다. 
...
이 경로를 그대로 쓰면 됩니다. 예를 들어 일부 예제에서는  
사이드바 아바타 이미지를 `/assets/img/site/avatar.png` 형태로 변경합니다.
...
1. **이미지를 `_posts` 안에 섞어 넣는 경우**  
   ...
   경로도 `/assets/...` 로 통일하는 편이 좋습니다. 

2. **경로 앞의 `/` 를 빼먹는 경우**  
   → 상대경로가 되면서 GitHub Pages에서 실제 경로와 달라질 수 있습니다.  
   가능하면 항상 `/assets/...` 형태를 사용합니다. 

3. **대표 이미지와 본문 이미지를 같은 설정으로 헷갈리는 경우**  
   → 대표 이미지는 `image.path`, 본문 이미지는 Markdown으로 분리해서 생각하면 덜 헷갈립니다. 

4. **폴더 이름·파일 이름에 공백, 한글, 괄호를 많이 쓰는 경우**  
   → URL 인코딩 문제와 경로 오류의 원인이 될 수 있습니다.  
   가능하면 `kebab-case` 영어 이름만 사용하는 것이 좋습니다. 
...
- `assets/css`, `assets/js`, `assets/img` 로 기본 분리하기  
- `assets/img/posts/글폴더/` 구조로 포스트 이미지를 관리하기  
- 대표 이미지는 `image.path`로, 본문 이미지는 Markdown으로 넣기  
- 공용 이미지는 `assets/img/site/` 에 모으기  
- 필요하면 `media_subpath`로 경로를 짧게 관리하기  
