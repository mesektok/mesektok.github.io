...
즉, 이미지 자체 문제라기보다  
**경로와 설정 문제**인 경우가 대부분입니다. [web:443][web:468]
...
이미지가 안 보일 수 있습니다. Jekyll에서도 루트 기준 경로와 baseurl 처리 문제는 이미지 미표시의 흔한 원인입니다. [web:469]
...
Chirpy 관련 이슈에서도  
실제 파일명과 경로가 다르거나 확장자가 다른 경우가 문제 원인으로 지적되었습니다. [web:443]
...
브라우저가 잘못된 URL처럼 처리한 사례도 있었습니다. [web:443]
...
버전에 따라 `img_path` 와 `image.path` 조합이 대표 이미지 미리보기에서 다르게 동작했던 사례가 있었습니다. [web:468]
...
본문 이미지에서는 보이는데  
대표 이미지 미리보기에서는 안 뜨는 문제가 보고된 적이 있습니다. [web:468]
...
Chirpy 이슈에서도  
포스트 본문 이미지는 보이지만 공유용 preview image가 정상적으로 처리되지 않던 사례가 있었습니다. [web:427][web:466]
...
사이트 내부에서 보이는 것과 다르게 동작할 수 있습니다. [web:427]
